...
## W27-FIX-13 · The owner's generated Acoperișuri picture on the service slot, under W27-R-19, 2026-09-23

Branch `w27/w27-fix-13-svc-acoperisuri`, stacked on W27-FIX-12.

**The owner's instruction, recorded verbatim as W27-R-19.** `/Users/ivan/svc-acoperisuri@2x_v2.jpg`
(2390x1792) went through `scripts/process-photos.js` as `photos-raw/svc-acoperisuri.jpg`, the slot's
own intake: centre crop to 4:3, `public/img/svc-acoperisuri.jpg` 800x600 (89KB) and
`svc-acoperisuri@2x.jpg` 1600x1200 (349KB), both stripped (the intake left an Exif block; exiftool
removed it; gate 17 reads clean). The raw copy was not committed, as the folder's README says.
**It renders everywhere the old file did**, because the path is the slot's: the homepage services
grid and the Acoperișuri page hero, RO and RU (four built pages name it).

**Provenance.** The two rows leave the legacy fingerprint (the bytes changed, so the legacy status
they had is gone by the R-W amendment's own rule) for W25-R3's approved origin: licence "AI
generated for Rapid Construct, generated by the owner, tool not named, 2026-09-23 (owner
instruction W27-R-19)", licence URL "n/a, generated by the owner". `scripts/check-asset-provenance.js`
accepted only an https URL, "supplier permission: ..." or "n/a, generated in this repo" on a
non-legacy row, because no owner-generated image had ever been installed; it now accepts "n/a,
generated by the owner" exactly when the licence cell names that origin, and nothing else.

## W27-FIX-14 · Pictures appear at once and the scroll stays smooth, under W27-R-20, 2026-09-23

Branch `w27/w27-fix-14-image-speed`, stacked on W27-FIX-13.

**The owner's instruction, recorded verbatim as W27-R-20. Measured before anything moved**, on the
built tree with the session's own instruments: a retina visitor's picture bytes (Chrome at device
pixel ratio 2, 1440 wide, cache off, scrolled to the bottom so every lazy picture is requested,
summed from the resource timings) and the rendered width of every picture at 1440, 1101, 768 and
390. Before: the homepage requested 31 pictures, **4,905KB**, the six largest being the service
cards' and step pictures' 2x files at 345 to 389KB each for boxes 366 and 207px wide; the roofing
page 121 pictures, 7,363KB.

**Two changes, both inside the standing rules.**

1. **The reveal observer reaches 25% of the viewport below the fold** (`rootMargin: '0px 0px 25% 0px'`,
   threshold 0, was threshold 0.1 with no margin), so a card is revealed before it scrolls into
   view rather than after a tenth of it is on screen. Still one IntersectionObserver, `unobserve`
   on first fire, no scroll handler, 380ms and 16px unchanged, reduced motion untouched: section 1
   holds as written.
2. **Card pictures are described by width, not by density.** The service cards (`srcset 800w,
   1600w`, `sizes` 366px at 1200 and up, 31vw at 900 and up, 96vw below), the five step pictures
   (900w, 1800w; 207px, 17vw, 96vw) and the four roofing offer cards (600w, 1200w; 255px, 21vw,
   90vw), every figure the measured width. A 2x desktop screen now takes the 1x file for each of
   them; the heroes (562px wide, the largest paint) keep "1x, 2x".

**After**, same instruments: homepage **2,905KB** (from 4,905, the same 31 pictures), roofing page
**6,927KB** (from 7,363). Lighthouse desktop (density 1, median of three): homepage 99 before and
99 after, roofing page 94 [94 94 96] before and 94 [94 97 93] after, the bimodal page W26-14
recorded, unchanged as expected because Lighthouse's desktop preset audits at density 1 and the
saving is the retina visitor's. **Not done, and why**: no file was re-encoded (the service and step
files are legacy-fingerprinted by R-W's amendment, so a re-encode would need a licence for each)
