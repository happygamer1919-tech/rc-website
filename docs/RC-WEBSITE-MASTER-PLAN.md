...
| Header | ~~**White bar carrying a dark pill.** Solid, opaque, fixed.~~ **AMENDED (W28-02, wave 27 design pass, under W27-R-07): a transparent band carrying a dark pill that floats on a deep soft shadow; the page scrolls behind it. The band stays fixed at its height because the body spacer is that height.** The rest of this cell is the earlier state: The bar is `#FFFFFF` with a 1px `--line` bottom border; the nav, phone, CTA and language switcher sit inside a ~~`#141414`~~ `#1C1917` (AMENDED W28-01) pill with a 999px radius. Not transparent, and not floating: the bar is fixed and `<body>` carries a constant spacer. *Amended 2026-09-01, W8-04. This row previously read "Not transparent, not a floating dark pill", which the built header contradicted from phase 1 onward. Ratified by Ivan; see DECISIONS.md.* |
...
- ~~Page under 9,000px desktop.~~ **AMENDED: superseded by R-I and then by R-J (DECISIONS.md, W12-04, amended by W12-10). The acceptance criterion is the per-locale budget held in R-J, not one flat figure, and RU sits above 9,000px under it by design. See the amendment at line 121.** **AMENDED (W14-13): the per-locale budgets are now held in ruling R-Y.**
