...
for (const { line, c } of rows) {
  ...
  if (!claimsLegacy && !claimsClient && onDisk && !isLegacy) {
    /* W27-FIX-13 (owner instruction W27-R-19): the owner's own generated image is W25-R3's approved
       origin ("AI generated for Rapid Construct", the sentence gate 19 holds), and it has no licence
       URL to give: the owner made it. That is the one other origin that may say so, and only when
       the licence cell names the origin, so the phrase cannot launder anything else. */
    const ownerGenerated = /AI generated for Rapid Construct/.test(licence) && /^n\/a, generated by the owner/.test(licenceUrl);
    if (!ownerGenerated && !/^https:\/\/\S+/.test(licenceUrl) && !/^supplier permission:\s*\S/.test(licenceUrl) && !/^n\/a, generated in this repo/.test(licenceUrl)) {
