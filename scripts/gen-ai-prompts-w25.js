...
L.push('');
L.push('## The one decision this pack cannot make for you');
L.push('');
L.push(`**${unknown} of the ${entries.length} entries are for products whose appearance the records do not describe.**`);
L.push('A lamp called `K1207` and a moulding called `RED 17` are a code and a price per metre.');
L.push('Nothing in the data says what shape or colour they are. A generated picture of those is');
L.push('**illustrative of the type and is not a portrait of that item**: a buyer choosing a');
L.push('moulding by its profile could order the wrong one.');
L.push('');
L.push('W25-R3 permits a generated image on a product slot, so the pack produces these entries');
L.push('rather than deciding for you. Each one is marked `appearance: not in the records`. Three');
L.push('ways to go, and they can be mixed:');
L.push('');
L.push('- **Generate them anyway**, accepting an illustrative picture. Fastest, fills the most slots.');
L.push('- **Photograph them in the shop.** They are physical items you stock. A phone photograph on a');
L.push('  plain surface beats a generated one, and it is the real product.');
L.push('- **Leave them grey** and fill the rest.');
L.push('');
...
for (const g of GROUP_ORDER) {
  ...
  for (const e of list) {
    ...
    L.push(`- **Appearance:** ${appearanceKnown(row, product) ? 'described by the ledger row or the record' : '**not in the records**, see the note above'}`);
    L.push('');
    L.push('```');
